...
@Immutable data class MapLevel(val number: Int, val state: LevelState, @param:StringRes val expectation: Int, val preview: KanaPreview? = null, val left: AdjacentType? = null, val right: AdjacentType? = null, val lessonTitle: String? = null) { val displayNumber get() = number.toString().padStart(2, '0') }
...
internal object MapFixtures {
    private val layout = listOf<MapRow>(
    ...
    fun rows(completed: Set<Int>): List<MapRow> {
        val current = PrototypeLessons.lessons.firstOrNull { it.id !in completed }?.id
        return layout.map { row ->
            if (row !is MapRow.Level) row else {
                val id = row.level.number
                MapRow.Level(row.level.copy(state = when {
                    PrototypeLessons.lesson(id) == null -> LevelState.Locked
                    id in completed -> LevelState.Completed
                    id == current -> LevelState.Current
                    isLessonUnlocked(id, completed) -> LevelState.Available
                    else -> LevelState.Locked
                }, preview = null, lessonTitle = PrototypeLessons.lesson(id)?.title))
            }
        }
    }
    fun level(number: Int?, completed: Set<Int>) = rows(completed).filterIsInstance<MapRow.Level>().firstOrNull { it.level.number == number }?.level
}
